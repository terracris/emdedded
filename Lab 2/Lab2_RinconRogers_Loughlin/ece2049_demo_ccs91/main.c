...
// TODO CHANGE:
enum state {welcome, countdown, playNotes, checkError, loser, winner};
...
void playNote(struct Note note) {
    ...
    int durationOffsetPos = (note.duration * SECINCLKTKS) + 50;
    int durationOffsetNeg = (note.duration * SECINCLKTKS) - 50;
...
void won() {
    Graphics_clearDisplay(&g_sContext); // Clear the display
        ...
        // Write some text to the display
        Graphics_drawStringCentered(&g_sContext, "WINNER", AUTO_STRING_LENGTH, 48, 15, TRANSPARENT_TEXT);

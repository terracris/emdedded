...
void DACInit() {
    //Assume reference voltage for DAC is 3.3V
    //Configure LDAC and CS for digital IO outputs

    DAC_PORT_LDAC_SEL &= ~DAC_PIN_LDAC;
    DAC_PORT_LDAC_DIR |= DAC_PIN_LDAC;
    DAC_PORT_LDAC_OUT |= DAC_PIN_LDAC;   //default LDAC = 1

    DAC_PORT_CS_SEL &= ~DAC_PIN_CS;
    DAC_PORT_CS_DIR |= DAC_PIN_CS;
    DAC_PORT_CS_OUT |= DAC_PIN_CS;     //default CS = 1

}

void DACSetValue(unsigned int dac_code) {

    //Start the SPI transmission by asserting CS = 0 (active low)
    DAC_PORT_CS_OUT &= ~DAC_PIN_CS;

    dac_code |= 0x3000;

    uint8_t lo_byte = (unsigned char) (dac_code & 0x00FF);
    uint8_t hi_byte = (unsigned char) ((dac_code & 0xFF00) >> 8); //shift byte

    // first we send the hi_byte because SPI uses MSB
    ...
    //We are done transmitting, so we set CS back to high (CS=1)
    DAC_PORT_CS_OUT |= DAC_PIN_CS;     //default CS = 1

    //Turns on output for DAC
    DAC_PORT_LDAC_OUT &= ~DAC_PIN_LDAC;   //set LDAC = 0
    __delay_cycles(10);
    DAC_PORT_LDAC_OUT |= DAC_PIN_LDAC;   //set LDAC = 1 (this turns off output?)
